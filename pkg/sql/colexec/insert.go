...
type vectorInserter struct {
	...
	mutationQuota int
	// If we are an implicit transaction and auto commit is true we'll commit
	// the last batch.
	txnImplicit bool
	autoCommit  bool
}
...
func (v *vectorInserter) Next() coldata.Batch {
	...
	enc := colenc.MakeEncoder(v.flowCtx.Codec(), v.desc, &v.flowCtx.Cfg.Settings.SV, b, v.insertCols, v.flowCtx.GetRowMetrics(), partialIndexColMap,
		func() bool {
			return kvba.Batch.ApproximateMutationBytes() > v.mutationQuota
		})
